...
$ cd flyingroutes/
```
#### Optional: on Linux systems (since 2.6.39), you might need to update the ICMP parameters to allow ICMP sockets creation (***flyingroutes*** uses SOCK_DGRAM ICMP sockets to UDP or ICMP send messages) for a given range of the groups ID as by default no group is allowed to do so
```
